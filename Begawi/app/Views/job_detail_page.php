            ...
            <div class="col-lg-8 pr-lg-5">
                ...
                <div class="job-header-meta mb-4">
                    <span><i class="bi bi-geo-alt-fill"></i>
                        <?= esc($job->location_name ?? 'Banjarmasin Selatan') ?></span>
                    <span><i class="bi bi-briefcase-fill"></i>
                        <?= esc($job->job_type ?? 'Penuh Waktu (Full Time)') ?></span>
                ...
                <p><?= isset($job->description) ? nl2br(esc($job->description)) : 'Kami mencari seorang Web Developer yang bersemangat dan berpengalaman...' ?>
                </p>
                ...
                <p><?= isset($job->application_instructions) ? nl2br(esc($job->application_instructions)) : 'Anda dapat melamar posisi ini langsung melalui website kami atau mengirimkan lamaran via email.<br><br>Untuk lamaran via email, kirimkan CV terbaru dan portofolio Anda ke: karir@ciptasolusi.com dengan subjek email <strong>‘Lamaran Web Developer - [Nama Anda]’</strong>.' ?>
                </p>
            </div>
            ...
            <div class="col-lg-4">
                <div class="summary-card">
                    ...
                    <ul>
                        ...
                        <li><strong>Tipe Pekerjaan:</strong> <?= esc($job->job_type ?? 'Penuh Waktu (Full Time)') ?>
                        </li>
                        <li>
                            <strong>Gaji:</strong>
                        ...
                        <li><strong>Batas Waktu Lamaran:</strong>
                            <?= isset($job->application_deadline) ? date('j F Y', strtotime($job->application_deadline)) : '3 Juli 2025' ?>
                        </li>
                    </ul>
                </div>
                ...
                <div class="summary-card mt-4">
                    ...
                    <p class="company-profile">
                        <?= esc($job->company_profile ?? 'PT Cipta Solusi adalah perusahaan teknologi...') ?>
                    </p>
                </div>
                ...
                <div class="mt-4">
                    <?php if (function_exists('session') && session()->get('isLoggedIn') && session()->get('role') === 'jobseeker'): ?>
                        ...
                        <?php endif; ?>
                    <?php else: ?>
                        <a href="/login?redirect=<?= function_exists('current_url') ? current_url() : '' ?>"
                            class="btn btn-apply btn-block text-center">
                            <i class="bi bi-cursor-fill"></i> Lamar dari Web Ini
                        </a>
                    <?php endif; ?>
                    ...
                    ?>
                    <a href="mailto:<?= esc($contact_email) ?>?subject=<?= rawurlencode($email_subject) ?>"
                        class="btn btn-apply-email btn-block text-center mt-2">
