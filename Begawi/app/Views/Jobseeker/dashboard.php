...
<div class="container my-5">
    ...
    <div class="profile-card-main shadow-sm mb-5">
        <div class="profile-avatar">
             <img src="<?= !empty($profile->profile_picture_path) ? base_url('uploads/avatars/' . esc($profile->profile_picture_path)) : 'https://cdn-icons-png.flaticon.com/512/149/149071.png' ?>" alt="Foto Profil">
        </div>

        <div class="profile-info">
            <h2><?= esc($profile->user_fullname ?? session()->get('fullname')) ?></h2>
            <p class="contact-info">
                <span><i class="bi bi-geo-alt-fill"></i><?= esc($profile->location_name ?? 'Domisili belum diisi') ?></span> | 
        ...
        <div class="profile-actions">
            ...
            <?php if (!empty($profile->resume_path)): ?>
                <a href="<?= base_url('uploads/resumes/' . esc($profile->resume_path)) ?>" class="btn" target="_blank"><i class="bi bi-file-earmark-text-fill"></i> Lihat CV</a>
            <?php endif; ?>
        </div>
    </div>
    <?php endif; ?>
                    ...
                    <div class="activity-item">
                        ...
                        <div class="activity-status">
                             <?php
                                ...
                                $status_text = ($status === 'approved') ? 'Accepted' : ucfirst($status);

                                // Mapping status ke class CSS baru
                                $status_class = 'status-reviewed'; // default
                                if (in_array($status, ['pending'])) $status_class = 'status-pending';
                                if (in_array($status, ['accepted', 'approved'])) $status_class = 'status-accepted';
                                if (in_array($status, ['rejected'])) $status_class = 'status-rejected';
                            ?>
