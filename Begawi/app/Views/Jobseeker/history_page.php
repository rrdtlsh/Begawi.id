...
<head>
    ...
    <style>
        ...
        .item-metadata .type-label {
            display: flex;
            align-items: center;
            margin-right: 15px; /* Jarak antara tipe dan tanggal */
        }
            ...
            <?php if (!empty($history)): ?>
                ...
                <div class="application-item">
                    ...
                    <div class="flex-grow-1">
                        <h6 class="mb-0 font-weight-bold"><?= esc($item->job_title ?? $item->title ?? 'Judul Tidak Ditemukan') ?></h6>
                        <small class="text-muted"><?= esc($item->company_name ?? 'Oleh: '.($item->penyelenggara ?? 'N/A')) ?></small>
                        ...
                        <div class="item-metadata">
                            <?php if (isset($item->job_title)): // Ini adalah Lamaran Kerja ?>
                                ...
                                <div class="date-label">
                                    ...
                                    Diajukan: <?= isset($item->applied_at) ? date('d M Y', strtotime($item->applied_at)) : 'N/A' ?>
                                </div>
                            <?php else: // Ini adalah Pendaftaran Pelatihan ?>
                                ...
                                <div class="date-label">
                                    <i class="bi bi-calendar3 mr-1"></i>
                                    <?php // PERBAIKAN: Menambahkan pengecekan sebelum menampilkan tanggal ?>
                                    Didaftar: <?= isset($item->enrolled_at) ? date('d M Y', strtotime($item->enrolled_at)) : 'N/A' ?>
                                </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <?php 
                        ...
                        $status_text = ($item->status ?? 'unknown') === 'approved' ? 'Diterima' : ucfirst($item->status ?? 'Unknown');
                    ?>
                    <div>
