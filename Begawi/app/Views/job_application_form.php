...
<body>
    <div class="main-container">
        ...
        <div class="job-summary mb-5">
            ...
            <p class="company-name">di <?= esc($job->company_name ?? 'Jasa PT Cipta Solusi') ?></p>
        </div>
        ...
        <form action="/lamar/job/<?= $job->id ?? '' ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" id="fullname" class="form-control" value="<?= esc(session()->get('fullname') ?? '') ?>" readonly>
            </div>
            <div class="form-group">
                <label for="email">Alamat Email</label>
                <input type="email" id="email" class="form-control" value="<?= esc(session()->get('email') ?? '') ?>" readonly>
            </div>
            <div class="form-group">
                <label for="phone">Nomor Telepon (Opsional)</label>
                <input type="text" id="phone" class="form-control" value="<?= esc($profile->phone ?? '') ?>" placeholder="Contoh: 081234567890" readonly>
            </div>
